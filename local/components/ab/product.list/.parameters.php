 ...
 */
CBitrixComponent::includeComponentClass('ab:product.list');
...
$arComponentParameters = array(
	...
	'PARAMETERS' => array(
		...
		"PREVIEW_IMG_WIDTH" => [
			"PARENT" => "EXTENDED_SETTINGS",
			"NAME" => "Ширина фотки превью"
		],
		"PREVIEW_IMG_HEIGHT" => [
			"PARENT" => "EXTENDED_SETTINGS",
			"NAME" => "Высота фотки превью"
		],
		"DETAIL_IMG_WIDTH" => [
			"PARENT" => "EXTENDED_SETTINGS",
			"NAME" => "Ширина детальной фотки"
		],
		"DETAIL_IMG_HEIGHT" => [
			"PARENT" => "EXTENDED_SETTINGS",
			"NAME" => "Высота детальной фотки"
		],
		"SECTION_DATA" => ["NAME "=> "Данные категории"]
	),
);
...
if ($catalogIncluded)
{
	if (!empty($filterDataValues))
	{
		$arComponentParameters['PARAMETERS']['CUSTOM_FILTER'] = array(
			...
			'TYPE' => 'CUSTOM',
			'JS_FILE' => Dress\Catalog\ProductList::getSettingsScript($componentPath, 'filter_conditions'),
